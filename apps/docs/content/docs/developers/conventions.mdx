...
- Migration files: `NNN_descriptive_name.up.sql` + `.down.sql` — always provide both directions. Numbers are zero-padded to three digits (applied in lexicographic filename order). **Fork-local migrations live in a reserved `9000+` band** so they never collide with upstream's climbing sequence — see `server/migrations/README.md`.
