...
func writeAvailableCommands(b *strings.Builder) {
	...
	b.WriteString("- `multica repo refresh <url>` — force the repo cache to fetch the latest refs now (the cache otherwise refreshes ~60s). Use it when a commit you know just landed isn't yet visible to `multica repo checkout` or `git fetch`; then re-checkout or `git fetch origin && git reset --hard origin/<branch>`.\n\n")
...
func writeRepositories(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("**If the snapshot looks stale.** The platform caches each repo and refreshes it on a fixed interval (~60s), so a just-landed commit may not be visible yet. Run `multica repo refresh <url>` to force an immediate fetch, then re-run `multica repo checkout` or `git fetch origin && git reset --hard origin/<branch>` inside the existing checkout. Don't clone the remote directly — the platform's gitconfig rewrites prevent it; refresh + recheckout is the supported path.\n\n")
