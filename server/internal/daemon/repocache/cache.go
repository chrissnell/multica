...
func gitEnv() []string {
	...
	// Fork seam: append the SSH insteadOf rewrite (see cache_fork.go).
	return appendForkGitConfig(env)
}
...
func gitFetch(barePath string) error {
	...
	// Fork seam: advance the bare's refs/heads/<default> for worker-pod ref
	// visibility. Non-fatal; see advanceLocalDefaultBranch in cache_fork.go.
