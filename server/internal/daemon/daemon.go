...
type repoCacheBackend interface {
	...
	// Fork methods (see cache_fork.go): Fetch backs the /repo/refresh handler;
	// CreateSharedClone backs controller-mode read-only-cache checkouts.
...
func (d *Daemon) resolveAuth() error {
	// Fork seam: honor headless MULTICA_TOKEN before the config file (see
	// applyForkTokenAuth in daemon_fork.go).
	if d.applyForkTokenAuth() {
		return nil
	}
...
func (d *Daemon) handleModelList(ctx context.Context, rt Runtime, requestID string) {
	...
	// Fork seam: payload built by BuildModelListPayload (daemon_fork.go), shared
	// with the k8s controller's heartbeat handler.
