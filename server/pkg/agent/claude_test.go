...
func TestClaudeExecuteClearsSessionIDOnNoConversationFound(t *testing.T) {
	...
	fakePath := filepath.Join(t.TempDir(), "claude")
	// Fake claude:
	//   - reads the single prompt frame so writeClaudeInput succeeds (stdin is
	//     kept open by the runtime for control frames, so a full drain would
	//     block until the run ends — read one line and move on)
