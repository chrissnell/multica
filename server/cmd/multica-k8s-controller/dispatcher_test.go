...
func TestDispatchOnce_CreatesPVCAndJobForClaimedTask(t *testing.T) {
	...
	dispatched, err := DispatchOnce(context.Background(), cli, k, "multica", "ghcr-pull", r, ClaudeBrokerOptions{}, RepoCacheOptions{}, GitHubTokenOptions{}, nil, BuildCacheOptions{})
	if err != nil {
		t.Fatal(err)
	}
	if !dispatched {
	...
	if _, err := DispatchOnce(context.Background(), cli, k, "multica", "ghcr-pull", r, ClaudeBrokerOptions{}, RepoCacheOptions{}, GitHubTokenOptions{}, nil, BuildCacheOptions{}); err != nil {
		t.Fatal(err)
	}
